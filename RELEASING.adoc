...
2. Builds native binaries for all platforms using `.github/workflows/build-native.yml`:
   - Linux x86_64 (mostly static binary, needs glibc >=2.35)
   - Linux aarch64 (mostly static binary, needs glibc >=2.35)
   - macOS aarch64 (Apple Silicon)
   - Windows x86_64
3. Creates distribution archives (`.zip`, `.tar.gz` and `.tar.zst`) for each platform
...
./mvnw -N -Prelease jreleaser:full-release -Djreleaser.github.token=${JRELEASER_GITHUB_TOKEN}
----
